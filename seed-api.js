...
async function seedDatabase() {
  ...
  for (const product of products) {
    try {
      ...
      if (response.ok) {
        console.log(`✅ Created: ${product.partId} - ${product.details}`);
      } else {
        console.log(`❌ Failed: ${product.partId} - ${await response.text()}`);
      }
    } catch (error) {
      console.error(`❌ Error creating ${product.partId}:`, error);
    }
  }
